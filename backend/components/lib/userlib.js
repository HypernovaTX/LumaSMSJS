...
export async function checkExistingUser(username, email) {
  if (!username && !email) {
    handleError("us0");
    return placeholderPromise(RESULT.badparam);
  }
  username = username ?? "";
  email = email ?? "";
  ...
  DB.buildSelect(userTable);
  if (!username) {
    whereQuery = `email = '${cleanEmail}'`;
  }
  if (!email) {
    whereQuery = `username = '${cleanUser}'`;
  }
...
export async function updateLoginCookie(uid, _response) {
  const token = jwt.sign({ uid }, CF.JWT_SECRET, {
